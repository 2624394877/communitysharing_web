...
import { getVerificationCodeByMail, resetMail, getUserInfo } from '@/core/network/service/auth';
...
const sendVerificationCode = async() => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(mail.value.email)) {
        ElNotification.error({
            title: '邮箱格式错误',
            message: '请输入正确的邮箱格式',
            offset: 100
...
const reset = async () => {
    try {
        const res = await resetMail(mail.value);
        if (res.success) {
            userInfo();
            ElNotification.success({
                title: '邮箱更换成功',
...
<template>
    <div class="web_reset_password">
        ...
        <div class="login">
            <input type="email" class="input_style phone" placeholder="请输入想要修改的邮箱" v-model="mail.email" />
        </div>
        <div class="login">
